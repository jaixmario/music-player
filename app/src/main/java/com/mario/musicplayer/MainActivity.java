...
public class MainActivity extends AppCompatActivity {
    ...
    private class SongAdapter extends ArrayAdapter<File> {
        private final List<File> songs;

        public SongAdapter(Context context, List<File> songs) {
        ...
        public View getView(int position, View convertView, @NonNull ViewGroup parent) {
            ...
            File songFile = songs.get(position);
            ...
            songTitle.setText(title != null ? title : songFile.getName());
    ...
    private int findIndexByPath(String path) {
        ...
        for (int i = 0; i < songList.size(); i++) {
            if (songList.get(i).getAbsolutePath().equals(path)) return i;
        }
        return -1;
    }
}